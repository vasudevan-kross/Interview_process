...
from app.services.video_proctoring_service import get_video_proctoring_service
...
class VideoSessionFinalizeRequest(BaseModel):
    webcam_duration: Optional[int] = None
    screen_duration: Optional[int] = None
...
@router.post("/video-session", summary="Create video proctoring session (public)")
async def create_video_session(request: VideoSessionCreateRequest):
    """
    Public endpoint - candidate creates a video proctoring session when starting their interview.
    Called before recording begins.
    """
    try:
        client = get_supabase()

        # Resolve org_id from submission
        sub_result = client.table("coding_submissions").select(
            "id, coding_interviews(org_id)"
        ).eq("id", request.submission_id).execute()

        if not sub_result.data:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Submission not found")

        org_id = sub_result.data[0]["coding_interviews"]["org_id"]
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create video session: {str(e)}")


@router.post("/video-session/{session_id}/upload-webcam-chunk", summary="Upload webcam chunk (public)")
async def upload_webcam_chunk(
    session_id: str,
    sequence: int,
    file: UploadFile = File(...),
):
    """Upload a webcam video chunk. Called every 30 seconds during interview."""
    try:
        chunk_data = await file.read()
        service = get_video_proctoring_service()
        result = service.upload_webcam_chunk(session_id, chunk_data, sequence)
        return result
    except Exception as e:
        logger.error(f"Error uploading webcam chunk: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to upload webcam chunk: {str(e)}")


@router.post("/video-session/{session_id}/upload-screen-chunk", summary="Upload screen chunk (public)")
async def upload_screen_chunk(
    session_id: str,
    sequence: int,
    file: UploadFile = File(...),
):
    """Upload a screen recording chunk. Called every 30 seconds during interview."""
    try:
        chunk_data = await file.read()
        service = get_video_proctoring_service()
        result = service.upload_screen_chunk(session_id, chunk_data, sequence)
        return result
    except Exception as e:
        logger.error(f"Error uploading screen chunk: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to upload screen chunk: {str(e)}")


@router.post("/video-session/{session_id}/finalize", summary="Finalize video session (public)")
async def finalize_video_session(
    session_id: str,
    request: VideoSessionFinalizeRequest,
    webcam_complete: bool = True,
    screen_complete: bool = True,
):
    """Finalize video proctoring session when interview ends."""
    try:
        service = get_video_proctoring_service()

        if webcam_complete:
            service.mark_upload_complete(session_id, "webcam")
        if screen_complete:
            service.mark_upload_complete(session_id, "screen")

        result = service.finalize_session(session_id, {
            "webcam_duration": request.webcam_duration,
            "screen_duration": request.screen_duration,
        })
        return result
    except Exception as e:
        logger.error(f"Error finalizing video session: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to finalize video session: {str(e)}")


@router.post("/video-session/{session_id}/face-event", summary="Log face detection event (public)")
async def log_face_event(session_id: str, request: FaceEventRequest):
    """Log a face detection event (absent or multiple faces) from client-side TF.js."""
    try:
        service = get_video_proctoring_service()
        return service.log_face_event(session_id, request.event_type, request.timestamp)
    except Exception as e:
        logger.error(f"Error logging face event: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to log face event: {str(e)}")


@router.get("/submissions/{submission_id}/video-playback", summary="Get video playback URLs")
async def get_video_playback_urls(
    ...
    try:
        service = get_video_proctoring_service()
        session = service.get_session_by_submission(submission_id)
        if not session:
            return {"session": None, "webcam_chunks": [], "screen_chunks": []}

        # Verify org owns the submission
        client = get_supabase()
        sub_result = client.table("coding_submissions").select(
            "coding_interviews(org_id)"
        ).eq("id", submission_id).execute()
        ...
        org_id = sub_result.data[0]["coding_interviews"]["org_id"]
        if str(org_id) != str(ctx.org_id):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized")

        return service.get_signed_urls(session["id"])
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting video playback URLs: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get playback URLs: {str(e)}")
